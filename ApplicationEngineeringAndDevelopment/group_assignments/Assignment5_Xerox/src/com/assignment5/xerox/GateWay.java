...
public class GateWay {
    ...
    public static void main(String args[]) throws IOException {
        ...
        while ((orderRow = orderReader.getNextRow()) != null) {
            ...
            orderRow = orderReader.getNextRow();
        }
       ...
       helper.bestProducts();
       helper.get3BestCustomers();
       helper.getThreeBestSalesPeople();
       helper.getModifiedPricing();
    }
    ...
    public static void generateData(String[] row) {
        ...
        // mapping of all products with orders
        if (DataStore.getInstance().getProductOfOrders().containsKey(productId)) {
            List<Order> oldOrders = DataStore.getInstance().getProductOfOrders().get(productId);
            oldOrders.add(o);
            DataStore.getInstance().getProductOfOrders().put(productId, oldOrders);
        } else {
            List<Order> firstOrder = new ArrayList<>();
            firstOrder.add(o);
            DataStore.getInstance().getProductOfOrders().put(productId, firstOrder);
        }
