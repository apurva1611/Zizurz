...
public class Lab7 {
    ...
    private Comment generateComment(String[] row){
        ...
        int commentUserId = Integer.parseInt(row[4]);
        int likes = Integer.parseInt(row[3]);
        int postId = Integer.parseInt(row[1]);
        String comment = row[5];
        
        Comment c = new Comment(commentId, commentUserId, postId, comment, likes);
        DataStore.getInstance().getComments().put(commentId, c);
        
        Map<Integer, User> users = DataStore.getInstance().getUsers();
        
        if(users.containsKey(commentUserId)){
            users.get(commentUserId).getComments().add(c);
        }
        return c;
    }
